...
const TYPES={living:{l:"Living / Library",c:"#F5F0E8"},kitchen:{l:"Kitchen / Service",c:"#FCEADB"},bedroom:{l:"Bedroom",c:"#DFEBF5"},bathroom:{l:"Bathroom",c:"#D5EDED"},utility:{l:"Utility / Storage",c:"#E9DDF2"},courtyard:{l:"Courtyard",c:"#DFECD6"},corridor:{l:"Corridor / Hall",c:"#EBE7DF"},iwan:{l:"Iwan / Outdoor",c:"#E5EED8"},entry:{l:"Entry",c:"#FFF5E6"},servants:{l:"Servants",c:"#F0DDE0"},stairs:{l:"Stairs / Elevator",c:"#DDD8D0"},garage:{l:"Garage / Parking",c:"#E0DDD8"},dining:{l:"Dining",c:"#F8F0E0"}};
const DT={single:{l:"Single Swing",w:.9},double:{l:"Double Swing",w:1.6},sliding:{l:"Sliding",w:1.2},pocket:{l:"Pocket",w:.9},fold:{l:"Bi-fold",w:1.4}};
const WT={standard:{l:"Standard",w:1.2},large:{l:"Large",w:2},small:{l:"Small",w:.6},floor:{l:"Floor-to-ceiling",w:1.5}};

const SNAP=.5,PAD=36,M2FT=3.28084,CM2IN=.393701;
const snpM=v=>Math.round(v/SNAP)*SNAP,snpF=v=>Math.round(v*10)/10;
const uid=()=>Date.now().toString(36)+Math.random().toString(36).slice(2,7);
const useMob=()=>{const[m,s]=useState(typeof window!=="undefined"?window.innerWidth<768:false);useEffect(()=>{const h=()=>s(window.innerWidth<768);window.addEventListener("resize",h);return()=>window.removeEventListener("resize",h)},[]);return m};
const dU=(m,u)=>u==="imperial"?Math.floor(m*M2FT)+"'"+Math.round((m*M2FT-Math.floor(m*M2FT))*12)+'"':m+"m";
const dA=(a,u)=>u==="imperial"?(a*M2FT*M2FT).toFixed(1)+" ft2":(a%1===0?a:a.toFixed(1))+"m2";
const wD=(cm,u)=>u==="imperial"?(cm*CM2IN).toFixed(1)+'"':cm+"cm";

async function LI(st){try{const r=await st.get("fp-index");return r?JSON.parse(r.value):[]}catch{return[]}}
async function SI(st,i){await st.set("fp-index",JSON.stringify(i))}
async function LP(st,id){try{const r=await st.get("fp-proj-"+id);return r?JSON.parse(r.value):null}catch{return null}}
async function SP(st,p){await st.set("fp-proj-"+p.id,JSON.stringify(p))}
async function DPj(st,id){await st.delete("fp-proj-"+id)}

function NumInput({value:v,onChange:oc,step:st,min:mn,style:sy}){
  const[l,sL]=useState(String(v));const[f,sF]=useState(false);
  useEffect(()=>{if(!f)sL(String(v))},[v,f]);
  const cm=()=>{const n=parseFloat(l);if(!isNaN(n)){const x=mn!=null?Math.max(mn,n):n;oc(x);sL(String(x))}else sL(String(v))};
  return <input type="number" step={st||1} min={mn} className="fps-input" style={sy} value={f?l:String(v)}
    onFocus={()=>{sF(true);sL(String(v))}} onBlur={()=>{cm();sF(false)}} onKeyDown={e=>{if(e.key==="Enter"){cm();e.target.blur()}}} onChange={e=>sL(e.target.value)}/>;
}
function TextInput({value:v,onChange:oc,style:sy,placeholder:ph}){
  const[l,sL]=useState(v);const[f,sF]=useState(false);
  useEffect(()=>{if(!f)sL(v)},[v,f]);
  return <input type="text" className="fps-input" style={sy} placeholder={ph} value={f?l:v}
    onFocus={()=>{sF(true);sL(v)}} onBlur={()=>{oc(l);sF(false)}} onKeyDown={e=>{if(e.key==="Enter"){oc(l);e.target.blur()}}} onChange={e=>sL(e.target.value)}/>;
}

const STY_ID="fps-s";
function injectCSS(){if(document.getElementById(STY_ID))return;const s=document.createElement("style");s.id=STY_ID;
...
function useHistory(initial){
  const hist=useRef([JSON.stringify(initial)]);const idx=useRef(0);
  const[val,setVal]=useState(initial);
  const push=useCallback(v=>{const s=JSON.stringify(v);if(s===hist.current[idx.current])return;
    hist.current=hist.current.slice(0,idx.current+1);hist.current.push(s);if(hist.current.length>50)hist.current.shift();else idx.current++;
    setVal(v)},[]);
  const undo=useCallback(()=>{if(idx.current>0){idx.current--;setVal(JSON.parse(hist.current[idx.current]))}},[]);
  const redo=useCallback(()=>{if(idx.current<hist.current.length-1){idx.current++;setVal(JSON.parse(hist.current[idx.current]))}},[]);
  const canUndo=idx.current>0,canRedo=idx.current<hist.current.length-1;
...
function getSnapGuides(rooms,movingId,x,y,w,h,pw,ph){
  const guides=[],T=.3;const edges={l:x,r:x+w,cx:x+w/2,t:y,b:y+h,cy:y+h/2};
  rooms.forEach(rm=>{if(rm.id===movingId)return;
    const re={l:rm.x,r:rm.x+rm.w,cx:rm.x+rm.w/2,t:rm.y,b:rm.y+rm.h,cy:rm.y+rm.h/2};
    // vertical guides (x alignment)
    [["l","l"],["r","r"],["l","r"],["r","l"],["cx","cx"]].forEach(([a,b])=>{
      if(Math.abs(edges[a]-re[b])<T)guides.push({type:"v",pos:re[b],from:Math.min(edges.t,re.t),to:Math.max(edges.b,re.b)})});
    // horizontal guides (y alignment)
    [["t","t"],["b","b"],["t","b"],["b","t"],["cy","cy"]].forEach(([a,b])=>{
      if(Math.abs(edges[a]-re[b])<T)guides.push({type:"h",pos:re[b],from:Math.min(edges.l,re.l),to:Math.max(edges.r,re.r)})});
  });
  return guides;
}
...
function RoomSVG({rm,sc,sel,onPD}){
  const rx=PAD+rm.x*sc,ry=PAD+rm.y*sc,rw=rm.w*sc,rh=rm.h*sc,cl=TYPES[rm.type]?.c||"#EEE",a=rm.w*rm.h;
  const t=rm.w<2||rm.h<2,m=rm.w<1.5||rm.h<1.5;
  const f1=m?5.5:t?7:Math.min(9,rw/Math.max(rm.name.length,1)*1.4),f2=m?4:t?5:6;
...
function DoorSVG({el,sc,sel,onPD,wp}){
  const w=el.width*sc,px=PAD+el.x*sc,py=PAD+el.y*sc,isH=el.orient==="h";
  const gW=isH?w:wp*2.5,gH=isH?wp*2.5:w,gx=isH?px:px-wp*1.25,gy=isH?py-wp*1.25:py;
  const dt=el.doorType||"single";let d;
  if(dt==="single"){const r=w;d=isH?<g><line x1={px} y1={py} x2={px+w} y2={py} stroke="#8B6914" strokeWidth={2}/><path d={"M"+px+","+py+" A"+r+","+r+" 0 0,1 "+(px+r*.866)+","+(py-r*.5)} fill="none" stroke="#8B6914" strokeWidth={.8} strokeDasharray="3,2"/></g>:<g><line x1={px} y1={py} x2={px} y2={py+w} stroke="#8B6914" strokeWidth={2}/><path d={"M"+px+","+py+" A"+r+","+r+" 0 0,0 "+(px+r*.5)+","+(py+r*.866)} fill="none" stroke="#8B6914" strokeWidth={.8} strokeDasharray="3,2"/></g>}
  else if(dt==="double"){const h=w/2;d=isH?<g><line x1={px} y1={py} x2={px+w} y2={py} stroke="#8B6914" strokeWidth={2}/><path d={"M"+px+","+py+" A"+h+","+h+" 0 0,1 "+(px+h*.707)+","+(py-h*.707)} fill="none" stroke="#8B6914" strokeWidth={.8} strokeDasharray="3,2"/><path d={"M"+(px+w)+","+py+" A"+h+","+h+" 0 0,0 "+(px+w-h*.707)+","+(py-h*.707)} fill="none" stroke="#8B6914" strokeWidth={.8} strokeDasharray="3,2"/></g>:<g><line x1={px} y1={py} x2={px} y2={py+w} stroke="#8B6914" strokeWidth={2}/><path d={"M"+px+","+py+" A"+h+","+h+" 0 0,0 "+(px+h*.707)+","+(py+h*.707)} fill="none" stroke="#8B6914" strokeWidth={.8} strokeDasharray="3,2"/></g>}
...
function Editor({project,onBack,st}){
  const{val:proj,push:pushH,undo,redo,canUndo,canRedo}=useHistory(project);
  const[fIdx,setFIdx]=useState(0);
  const[selId,setSelId]=useState(null);const[selCat,setSelCat]=useState("room");
  const[tool,setTool]=useState("select");const[drag,setDrag]=useState(null);
  const[pan,setPan]=useState("props");const[panO,setPanO]=useState(false);
  const[showAF,setShowAF]=useState(false);const[nfn,setNfn]=useState("");
  const[guides,setGuides]=useState([]);
  const svgR=useRef(null);const stR=useRef(null);const mob=useMob();

  useEffect(()=>{injectCSS()},[]);

  const fl=proj.floors[fIdx]||proj.floors[0];const rms=fl?.rooms||[];const els=fl?.elements||[];
  const pw=proj.plotWidth,ph=proj.plotHeight,wt=proj.wallThickness||.2,un=proj.units||"metric";
  const md=Math.max(pw,ph,1);const sc=Math.max(10,Math.min(28,(mob?(window.innerWidth-40):520)/md));
  const cvw=pw*sc+PAD*2,cvh=ph*sc+PAD*2,wp=wt*sc;

  const autoSave=useCallback(p=>{if(stR.current)clearTimeout(stR.current);stR.current=setTimeout(()=>{const u={...p,updatedAt:new Date().toISOString()};SP(st,u);LI(st).then(ix=>{SI(st,ix.map(it=>it.id===u.id?{...it,name:u.name,updatedAt:u.updatedAt}:it))})},800)},[st]);
  const upd=useCallback(fn=>{const n=JSON.parse(JSON.stringify(proj));fn(n);n.floors.forEach(f=>{if(!f.elements)f.elements=[];if(!f.columns)f.columns=[];if(f.height==null)f.height=3});autoSave(n);pushH(n)},[proj,autoSave,pushH]);

  const gPt=e=>{const s=svgR.current,r=s.getBoundingClientRect();return{x:(e.clientX-r.left)*(cvw/r.width),y:(e.clientY-r.top)*(cvh/r.height)}};
  const selRoom=id=>{setSelId(id);setSelCat("room");setPan("props");if(mob)setPanO(true)};
  const selEl=id=>{setSelId(id);setSelCat("element");setPan("props");if(mob)setPanO(true)};
  const clr=()=>{setSelId(null);setSelCat("room");setGuides([])};
  const sRoom=selCat==="room"?rms.find(r=>r.id===selId):null;
  const sEl=selCat==="element"?els.find(e=>e.id===selId):null;

  const onRD=(e,id)=>{if(tool!=="select"||e.button!==0)return;e.preventDefault();selRoom(id);const pt=gPt(e),rm=rms.find(r=>r.id===id);if(!rm)return;setDrag({m:"move",id,c:"room",sp:pt,sr:{...rm}});svgR.current.setPointerCapture(e.pointerId)};
  const onHD=(e,id,h)=>{if(e.button!==0)return;e.preventDefault();const pt=gPt(e),rm=rms.find(r=>r.id===id);if(!rm)return;setDrag({m:"resize",id,c:"room",h,sp:pt,sr:{...rm}});svgR.current.setPointerCapture(e.pointerId)};
  const onED=(e,id)=>{if(tool!=="select"||e.button!==0)return;e.preventDefault();selEl(id);const pt=gPt(e),el=els.find(x=>x.id===id);if(!el)return;setDrag({m:"move",id,c:"el",sp:pt,sr:{...el}});svgR.current.setPointerCapture(e.pointerId)};

  const onPM=e=>{if(!drag)return;const pt=gPt(e),dx=(pt.x-drag.sp.x)/sc,dy=(pt.y-drag.sp.y)/sc,sr=drag.sr;
    if(drag.c==="room"){const n=JSON.parse(JSON.stringify(proj));const rm=n.floors[fIdx].rooms.find(r=>r.id===drag.id);if(!rm)return;
      if(drag.m==="move"){rm.x=Math.max(0,Math.min(pw-rm.w,snpM(sr.x+dx)));rm.y=Math.max(0,Math.min(ph-rm.h,snpM(sr.y+dy)));
        setGuides(getSnapGuides(rms,rm.id,rm.x,rm.y,rm.w,rm.h,pw,ph))}
      else{const h=drag.h;let nx=sr.x,ny=sr.y,nw=sr.w,nh=sr.h;
        if(h.includes("w")){nx=snpM(sr.x+dx);nw=sr.w-(nx-sr.x)}if(h.includes("e"))nw=snpM(sr.w+dx);
        if(h.includes("n")){ny=snpM(sr.y+dy);nh=sr.h-(ny-sr.y)}if(h.includes("s"))nh=snpM(sr.h+dy);
        if(nw<1){if(h.includes("w"))nx=sr.x+sr.w-1;nw=1}if(nh<1){if(h.includes("n"))ny=sr.y+sr.h-1;nh=1}
        nx=Math.max(0,nx);ny=Math.max(0,ny);if(nx+nw>pw)nw=pw-nx;if(ny+nh>ph)nh=ph-ny;
        rm.x=nx;rm.y=ny;rm.w=nw;rm.h=nh;setGuides(getSnapGuides(rms,rm.id,rm.x,rm.y,rm.w,rm.h,pw,ph))}
      n.floors.forEach(f=>{if(!f.elements)f.elements=[];if(!f.columns)f.columns=[]});autoSave(n);pushH(n)}
    if(drag.c==="el"){const n=JSON.parse(JSON.stringify(proj));const el=n.floors[fIdx].elements.find(x=>x.id===drag.id);if(!el)return;
      if(el.type==="column"){el.x=snpF(Math.max(0,Math.min(pw,sr.x+dx)));el.y=snpF(Math.max(0,Math.min(ph,sr.y+dy)))}
      else if(el.orient==="h"){el.x=snpF(Math.max(0,Math.min(pw-el.width,sr.x+dx)));el.y=snpF(Math.max(0,Math.min(ph,sr.y+dy)))}
      else{el.x=snpF(Math.max(0,Math.min(pw,sr.x+dx)));el.y=snpF(Math.max(0,Math.min(ph-el.width,sr.y+dy)))}
      n.floors.forEach(f=>{if(!f.elements)f.elements=[];if(!f.columns)f.columns=[]});autoSave(n);pushH(n)}};
  const onPU=()=>{setDrag(null);setGuides([])};

  const findWall=(mx,my)=>{let b={d:Infinity,x:mx,y:my,o:"h"};const ck=(wx,wy,o)=>{const d=o==="h"?Math.abs(my-wy):Math.abs(mx-wx);if(d<b.d)b={d,x:o==="h"?mx:wx,y:o==="h"?wy:my,o}};
    ck(mx,0,"h");ck(mx,ph,"h");ck(0,my,"v");ck(pw,my,"v");rms.forEach(r=>{ck(mx,r.y,"h");ck(mx,r.y+r.h,"h");ck(r.x,my,"v");ck(r.x+r.w,my,"v")});return b};

  const onBgDown=e=>{if(e.button!==0)return;const pt=gPt(e),mx=(pt.x-PAD)/sc,my=(pt.y-PAD)/sc;
    if(tool==="door"||tool==="window"){if(mx<0||my<0||mx>pw||my>ph)return;const sn=findWall(mx,my),id=uid(),w=tool==="door"?.9:1.2;
      const ne={id,type:tool,x:sn.o==="h"?snpF(Math.max(0,Math.min(pw-w,sn.x-w/2))):snpF(sn.x),y:sn.o==="h"?snpF(sn.y):snpF(Math.max(0,Math.min(ph-w,sn.y-w/2))),orient:sn.o,width:w};
      if(tool==="door")ne.doorType="single";else ne.winType="standard";upd(p=>{p.floors[fIdx].elements.push(ne)});selEl(id);return}
    if(tool==="column"){if(mx<0||my<0||mx>pw||my>ph)return;const id=uid();const ne={id,type:"column",x:snpF(mx),y:snpF(my),size:.3};
      upd(p=>{p.floors[fIdx].elements.push(ne)});selEl(id);return}
    clr();if(mob)setPanO(false)};

  const addRm=()=>{const id=uid();upd(p=>{p.floors[fIdx].rooms.push({id,name:"New Room",type:"living",x:0,y:0,w:4,h:3})});selRoom(id)};
  const dupRm=()=>{if(!sRoom)return;const id=uid();upd(p=>{p.floors[fIdx].rooms.push({...JSON.parse(JSON.stringify(sRoom)),id,name:sRoom.name+" Copy",x:Math.min(sRoom.x+1,pw-sRoom.w),y:Math.min(sRoom.y+1,ph-sRoom.h)})});selRoom(id)};
  const delSel=()=>{if(!selId)return;if(selCat==="room")upd(p=>{p.floors[fIdx].rooms=p.floors[fIdx].rooms.filter(r=>r.id!==selId)});else upd(p=>{p.floors[fIdx].elements=p.floors[fIdx].elements.filter(x=>x.id!==selId)});clr()};
  const addFl=()=>{const nm=nfn.trim()||"Floor "+(proj.floors.length+1);upd(p=>{p.floors.push({id:uid(),name:nm,rooms:[],elements:[],columns:[],height:3})});setFIdx(proj.floors.length);setShowAF(false);setNfn("")};
  const copyFl=()=>{upd(p=>{const src=JSON.parse(JSON.stringify(p.floors[fIdx]));src.id=uid();src.name=src.name+" (Copy)";src.rooms.forEach(r=>r.id=uid());src.elements.forEach(e=>e.id=uid());p.floors.push(src)});setFIdx(proj.floors.length)};
  const delFl=()=>{if(proj.floors.length<=1)return;upd(p=>{p.floors.splice(fIdx,1)});setFIdx(Math.max(0,fIdx-1));clr()};
  const expJ=()=>{const b=new Blob([JSON.stringify(proj,null,2)],{type:"application/json"});const a=document.createElement("a");a.href=URL.createObjectURL(b);a.download=(proj.name||"plan")+".json";a.click()};
  const printPlan=()=>{window.print()};

  const tA=rms.reduce((s,r)=>s+r.w*r.h,0);const plotA=pw*ph;const freeA=plotA-tA;
  const byType={};rms.forEach(r=>{byType[r.type]=(byType[r.type]||0)+r.w*r.h});
  const fh=fl?.height||3;
  // Stair calculator
  const stairRooms=rms.filter(r=>r.type==="stairs");
  const stairInfo=stairRooms.length>0?{riserH:fh/(Math.round(fh/.18)),numSteps:Math.round(fh/.18),treadD:.28,runLen:Math.round(fh/.18)*.28}:null;

  useEffect(()=>{const onK=e=>{
    if((e.ctrlKey||e.metaKey)&&e.key==="z"&&!e.shiftKey){e.preventDefault();undo();return}
    if((e.ctrlKey||e.metaKey)&&(e.key==="y"||(e.key==="z"&&e.shiftKey))){e.preventDefault();redo();return}
    if(!selId||["INPUT","SELECT","TEXTAREA"].includes(e.target.tagName))return;
    if(selCat==="room"){const st=e.shiftKey?1:.5;let h=false;
      if(e.key==="ArrowLeft"){upd(p=>{const r=p.floors[fIdx].rooms.find(r2=>r2.id===selId);if(r)r.x=Math.max(0,r.x-st)});h=true}
      if(e.key==="ArrowRight"){upd(p=>{const r=p.floors[fIdx].rooms.find(r2=>r2.id===selId);if(r)r.x=Math.min(pw-r.w,r.x+st)});h=true}
      if(e.key==="ArrowUp"){upd(p=>{const r=p.floors[fIdx].rooms.find(r2=>r2.id===selId);if(r)r.y=Math.max(0,r.y-st)});h=true}
      if(e.key==="ArrowDown"){upd(p=>{const r=p.floors[fIdx].rooms.find(r2=>r2.id===selId);if(r)r.y=Math.min(ph-r.h,r.y+st)});h=true}
      if(h)e.preventDefault()}
    if(e.key==="Delete"||e.key==="Backspace"){delSel();e.preventDefault()}
    if(e.key==="Escape"){setTool("select");clr()}
  };window.addEventListener("keydown",onK);return()=>window.removeEventListener("keydown",onK)},[selId,selCat,fIdx,pw,ph,proj]);
...
function Library({onOpen,projects,onNew,onDelete,loading}){
  const[showNew,setShowNew]=useState(false);const[name,setName]=useState("");const[pw,setPw]=useState(24);const[ph,setPh]=useState(25);const[wtCm,setWtCm]=useState(20);const[units,setUnits]=useState("metric");const mob=useMob();
  useEffect(()=>{injectCSS()},[]);
...
export default function FloorPlanStudio({storage:st}){
  const[view,setView]=useState("library");const[projects,setProjects]=useState([]);const[activeProj,setActiveProj]=useState(null);const[loading,setLoading]=useState(true);
  useEffect(()=>{LI(st).then(ix=>{setProjects(ix);setLoading(false)})},[]);
  const openP=async id=>{const p=await LP(st,id);if(p){setActiveProj(p);setView("editor")}};
  const newP=async({name,plotWidth,plotHeight,wallThickness,units:un})=>{const id=uid(),now=new Date().toISOString();
    const isImp=un==="imperial";const pwM=isImp?+(plotWidth/M2FT).toFixed(2):plotWidth;const phM=isImp?+(plotHeight/M2FT).toFixed(2):plotHeight;const wtM=isImp?(wallThickness/CM2IN/100):(wallThickness||.2);
    const proj={id,name,plotWidth:pwM,plotHeight:phM,wallThickness:wtM,units:un||"metric",createdAt:now,updatedAt:now,floors:[{id:uid(),name:"Ground Floor",rooms:[],elements:[],columns:[],height:3}]};
    await SP(st,proj);const ix=[...projects,{id,name,plotWidth:pwM,plotHeight:phM,wallThickness:wtM,units:un||"metric",createdAt:now,updatedAt:now}];await SI(st,ix);setProjects(ix);setActiveProj(proj);setView("editor")};
  const delP=async id=>{await DPj(st,id);const ix=projects.filter(p=>p.id!==id);await SI(st,ix);setProjects(ix)};
  const goB=()=>{LI(st).then(ix=>{setProjects(ix);setView("library");setActiveProj(null)})};
  if(view==="editor"&&activeProj) return <Editor project={activeProj} onBack={goB} st={st}/>;
  return <Library projects={projects} loading={loading} onOpen={openP} onNew={newP} onDelete={delP}/>;
}
